...
// // #include "cuda_fp16.h"              //include the half precision conversion and data movement library
...
//     // /* Declare variables for performing half precision computation */
//     // float input_value;
//     // float mask_value;
//     // float output_value;
//     // __half half_input_value;
//     // __half half_mask_value;
//     // __half half_output_value;       
...
//     //             for(int q = 0; q < K; q++){
//     //                 /* FP16 Optimization */
//     //                 // Retrieve input and mask values and convert to half precision
//     //                 input_value = in_4d(b, c, S*h+p, S*w+q);
//     //                 half_input_value = __float2half(input_value);
//     //                 mask_value = mask_4d(m, c, p, q);
//     //                 half_mask_value = __float2half(mask_value);
//     //                 // Perform half precision computation and accumulate half_output_value
//     //                 half_output_value += half_input_value * half_mask_value;
//     //             }
//     //         }
//     //     }
//     // // Convert half precision output value to float then store into output matrix
//     // output_value = __half2float(half_output_value);
//     // out_4d(b, m, h, w) = output_value;
//     // }
...
__global__ void conv_forward_kernel(float *output, const float *input, const float *mask, const int B, const int M, const int C, const int H, const int W, const int K,const int S)
{
    ...
    */
    ...
    /* Declare output value half precision variable for performing half precision computation */
    __half half_output_value = 0.0f;       
        ...
        for(int c = 0; c < C; c++){
            for(int p = 0; p < K; p++){
                for(int q = 0; q < K; q++){
                    /* FP16 Optimization */
                    // Retrieve input and mask values and convert to half precision
                    __half half_input_value = __float2half(in_4d(b, c, S*h+p, S*w+q));
                    __half half_mask_value = __float2half(mask_4d(m, c, p, q));
                    // Perform half precision computation and accumulate half_output_value
                    half_output_value += half_input_value * half_mask_value;
                }
            }
    ...
    // Convert half precision output value to float then store into output matrix
    out_4d(b, m, h, w) = __half2float(half_output_value);
    }
...
__host__ void GPUInterface::conv_forward_gpu_prolog(const float *host_output, const float *host_input, const float *host_mask, float **device_output_ptr, float **device_input_ptr, float **device_mask_ptr, const int B, const int M, const int C, const int H, const int W, const int K, const int S)
{
    ...
    // copy data to memory 
    cudaMemcpy(*device_input_ptr, host_input, size_of_input, cudaMemcpyHostToDevice);
    cudaMemcpy(*device_mask_ptr, host_mask, size_of_mask, cudaMemcpyHostToDevice);
}
