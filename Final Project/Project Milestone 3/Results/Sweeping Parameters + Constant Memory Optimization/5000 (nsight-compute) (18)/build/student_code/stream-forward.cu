...
__host__ void GPUInterface::conv_forward_gpu_prolog(const float *host_output, const float *host_input, const float *host_mask, float **device_output_ptr, float **device_input_ptr, float **device_mask_ptr, const int B, const int M, const int C, const int H, const int W, const int K, const int S)
{
    ...
    // pin host input and output memory 
    ...
    dim3 dimGrid(M, H_grid*W_grid, B/B);
    dim3 dimBlock(TILE_WIDTH, TILE_WIDTH, 1);

    // Declare and create streams (at least 3 for continuous pipelining)
    ...
    for(int i = 0; i < B; i++){
        ...
        cudaMemcpyAsync(*device_input_ptr + i*((B*C*H*W)/B), host_input + i*((B*C*H*W)/B), ((B*C*H*W)/B)*sizeof(float), cudaMemcpyHostToDevice, stream[i]);
        // launch kernel 
        conv_forward_kernel<<<dimGrid, dimBlock, 0, stream[i]>>>(*device_output_ptr + i*((B*M*H_out*W_out)/B), *device_input_ptr + i*((B*C*H*W)/B), *device_mask_ptr, B, M, C, H, W, K, S);
        // copy output back to host memory 
        cudaMemcpyAsync((float*) host_output + i*((B*M*H_out*W_out)/B), *device_output_ptr + i*((B*M*H_out*W_out)/B), ((B*M*H_out*W_out)/B)*sizeof(float), cudaMemcpyDeviceToHost, stream[i]);
    }
