...
__constant__ half weight_matrix[16000];                // initialize to a large size

/* Declare pointers to device memory for half input, output, and mask */
half *device_input_half_ptr;
half *device_output_half_ptr;
half *device_mask_half_ptr;

/* Tuning with Restrict and Loop Unrolling Optimization */
// Restrict Input, Output Pointers 
__global__ void conv_forward_kernel(half * __restrict__ output, const half * __restrict__ input, const int B, const int M, const int C, const int H, const int W, const int K,const int S)
{
    /*
...
/* Kernel for converting float values to half values */
__global__ void float_half_kernel(half *half_data, const float *float_data, const int size){
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if(i < size) half_data[i] = __float2half(float_data[i]);
}

/* Kernel for converting half values to float values */
__global__ void half_float_kernel(float *float_data, const half *half_data, const int size){
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if(i < size) float_data[i] = __half2float(half_data[i]);
}
...
__host__ void GPUInterface::conv_forward_gpu_prolog(const float *host_output, const float *host_input, const float *host_mask, float **device_output_ptr, float **device_input_ptr, float **device_mask_ptr, const int B, const int M, const int C, const int H, const int W, const int K, const int S)
{
    ...
    int size_of_input_float = B*C*H*W*sizeof(float);
    int size_of_output_float = B*M*H_out*W_out*sizeof(float);
    int size_of_mask_float = M*C*K*K*sizeof(float);

    // // pin host input and output memory; use size in bytes of input and output and default flag mode 
    // cudaHostRegister((void*) host_input, size_of_input_float, cudaHostRegisterDefault);
    // cudaHostRegister((void*) host_output, size_of_output_float, cudaHostRegisterDefault);

    // find half size of input, output, and mask
    int size_of_input_half = B*C*H*W*sizeof(half);
    int size_of_output_half = B*M*H_out*W_out*sizeof(half);
    int size_of_mask_half = M*C*K*K*sizeof(half);

    // allocate memory for input, output, and mask (float)
    cudaMalloc((void**) device_input_ptr, size_of_input_float);
    cudaMalloc((void**) device_output_ptr, size_of_output_float);
    cudaMalloc((void**) device_mask_ptr, size_of_mask_float);

    // allocate memory for input, output, and mask (half)
    cudaMalloc((void**) &device_input_half_ptr, size_of_input_half);
    cudaMalloc((void**) &device_output_half_ptr, size_of_output_half);
    cudaMalloc((void**) &device_mask_half_ptr, size_of_mask_half);

    // allocate host memory for mask_half in global memory
    half *host_mask_half_ptr = (half*) malloc(size_of_mask_half);

    // copy input and mask to device memory (float)
    cudaMemcpy(*device_input_ptr, host_input, size_of_input_float, cudaMemcpyHostToDevice);
    cudaMemcpy(*device_mask_ptr, host_mask, size_of_mask_float, cudaMemcpyHostToDevice);

    // launch kernels to convert float to half 
    dim3 dimGridMask(ceil(1.0*size_of_mask_half/(sizeof(half)*TILE_WIDTH)), 1, 1);
    dim3 dimBlockMask(TILE_WIDTH, 1, 1);
    float_half_kernel<<<dimGridMask, dimBlockMask>>>(device_mask_half_ptr, *device_mask_ptr, size_of_mask_half/sizeof(half));

    dim3 dimGridInput(ceil(1.0*size_of_input_half/(sizeof(half)*TILE_WIDTH)), 1, 1);
    dim3 dimBlockInput(TILE_WIDTH, 1, 1);
    float_half_kernel<<<dimGridInput, dimBlockInput>>>(device_input_half_ptr, *device_input_ptr, size_of_input_half/sizeof(half));

//    // unpin host input and output memory 
//     cudaHostUnregister((void*) host_input);

    /* Constant Memory Optimization */
    // copy mask back to host memory in order to copy it to constant memory
    cudaMemcpy(host_mask_half_ptr, device_mask_half_ptr, size_of_mask_half, cudaMemcpyDeviceToHost);
    // copy kernel mask values to constant memory
    cudaMemcpyToSymbol(weight_matrix, host_mask_half_ptr, size_of_mask_half);
}
...
__host__ void GPUInterface::conv_forward_gpu(float *device_output, const float *device_input, const float *device_mask, const int B, const int M, const int C, const int H, const int W, const int K, const int S)
{
    ...
    const int H_out = (H - K)/S + 1;      
    const int W_out = (W - K)/S + 1;      
    float H_grid = ceil(1.0*H_out/TILE_WIDTH);      // # of vertical tiles per output map
    float W_grid = ceil(1.0*W_out/TILE_WIDTH);      // # of horizontal tiles per output map
    // int H_grid = ((H_out -1)/TILE_WIDTH) + 1;      // # of vertical tiles per output map
    // int W_grid = ((W_out -1)/TILE_WIDTH) + 1;      // # of horizontal tiles per output map
    dim3 dimGrid(M, H_grid*W_grid, B);
    dim3 dimBlock(TILE_WIDTH, TILE_WIDTH, 1);

    conv_forward_kernel<<<dimGrid, dimBlock>>>(device_output_half_ptr, device_input_half_ptr, B, M, C, H, W, K, S);
}
...
__host__ void GPUInterface::conv_forward_gpu_epilog(float *host_output, float *device_output, float *device_input, float *device_mask, const int B, const int M, const int C, const int H, const int W, const int K, const int S)
{
    ...
    // calculate size of output (float)
    const int H_out = (H - K)/S + 1;      
    const int W_out = (W - K)/S + 1; 
    int size_of_output = B*M*H_out*W_out*sizeof(float);

    // convert output values from half to float 
    dim3 dimGridOutput(ceil(1.0*size_of_output/(sizeof(float)*TILE_WIDTH)), 1, 1);
    dim3 dimBlockOutput(TILE_WIDTH, 1, 1);
    half_float_kernel<<<dimGridOutput, dimBlockOutput>>>(device_output, device_output_half_ptr, size_of_output/sizeof(float));
    ...
    cudaFree(device_input_half_ptr);
    cudaFree(device_output_half_ptr);
    cudaFree(device_mask_half_ptr);
}
