...
void conv_forward_cpu(float *output, const float *input, const float *mask, const int B, const int M, const int C, const int H, const int W, const int K, const int S)
{
  ...
  for(int b = 0; b < B; ++b){
    // Loop through each output feature map
    for(int m = 0; m < M; m++){
      // Loop through row of output
      for(int h = 0; h < H_out; h++){
        // Loop through column of output
        for(int w = 0; w < W_out; w++){
          out_4d(b, m, h, w) = 0.0f;
          // Loop through each input feature map
          for(int c = 0; c < C; c++){
            // Loop through row of filter
            for(int p = 0; p < K; p++){
              // Loop through column of filter
              for(int q = 0; q < K; q++){  
